...
class ProductProduct(models.Model):
    ...
    topaz_lotname = fields.Char('Topaz Lot Name')
    topaz_expiredate = fields.Char('Topaz Expire Date')
    @api.model
        ...
        if name:
            ...
            if operator in positive_operators:
                ...
                if not product_ids:
                    ...
                    if (d) and (l):
                        self.env.cr.execute(
                            'update product_product set topaz_lotname=%s,topaz_expiredate=%s where id=%s',
                            (l, d, product_ids[0]))
                        self.env.cr.commit()
                    # print(rec)
